package jp.quadro.display
{
	...
	public class AbstractSprite extends Sprite
	{
		...
		public function AbstractSprite(container:DisplayObjectContainer = null, index:int = -1, key:String = '', group:String = '', id:uint = 0)
		{
		 ...
		 * @param
		 */
		public function drawRectangle(color:uint, x:Number, y:Number, width:Number, height:Number):Shape
		{
		...
		{
			try {
				if (!_listenerManager) throw new Error ('removeEventListenerは使用できません。');
			}
			catch (err:Error)
			{
				trace('removeEventListenerは使用できませんでした。');
			}
			finally 
			{
		...
		{
			if (!_listenerManager) throw new Error ('removeEventsForTypeは使用できません。');
			_listenerManager.removeEventsForType(type);
		}
		
		public function removeEventsForListener(listener:Function):void
		{
			if (!_listenerManager) throw new Error ('removeEventsForListenerは使用できません。');
			_listenerManager.removeEventsForListener(listener);
		}
		
		public function removeEventListeners():void
		{
			if (!_listenerManager) throw new Error ('removeEventListenersは使用できません。');
			_listenerManager.removeEventListeners();
		}
		
		public function getTotalEventListeners(type:String = null):uint
		{
			if (!_listenerManager) throw new Error ('getTotalEventListenersは使用できません。');
